...
description: |
  ...
  Use when:
  - About to make any non-trivial edit to a code file in codexray
  ...
  - Any change touching codexray/** or tests/**
...
## Worked example: editing `codexray/health_scorer.py`
...
PARALLEL BATCH (one message):
  → edit action=safe file_path="codexray/health_scorer.py" edit_type="add_feature"
  → health action=file file_path="codexray/health_scorer.py"
  → project action=smart file_path="codexray/health_scorer.py"
...
| Surface touched (in `codexray/`) | Codemap to refresh |
|---|---|
...
git diff --cached -- codexray/mcp/_tool_registry.py | grep -E '^\+.*\("(codegraph_|[a-z_]+",)'
```
...
```bash
uv run codexray codexray/health_scorer.py \
