...
```bash
uv run codexray --project-health --output-format toon
uv run codexray <file> --file-health --output-format toon
uv run codexray --overview --output-format toon
uv run codexray --dead-code --output-format toon
uv run codexray --watch-health --threshold-grade C  # daemon
```
