...
```bash
git -C /Users/aisheng.yu/git-private/codexray diff HEAD~2 HEAD --name-only
...
# Call 3
edit action=constraints path_filter="codexray/mcp/**"
# returns:
#   verdict: SAFE
#   violations: []
```
...
```bash
# Step 1 — Diff fan-out
uv run codexray --pr-review --change-impact-mode staged --output-format toon
uv run codexray --change-impact --change-impact-mode staged --agent-summary-only --output-format json
uv run codexray --check-constraints --constraint-path-filter "codexray/mcp/**" --output-format toon

# Step 2 — Per-file
uv run codexray <file> --safe-to-edit --edit-type refactor --output-format json
