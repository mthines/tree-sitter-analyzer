...
Run on `/Users/aisheng.yu/git-private/codexray`:

```bash
# Parallel CLI batch (humans):
uv run codexray --project-health --max-files 20 --output-format json > /tmp/health.json
uv run codexray --dead-code --output-format json > /tmp/dead.json
uv run python -c "
import sqlite3
sql = '''
...
top_5_refactor_queue:
  ...
  - rank: 2
    file: codexray/languages/python_plugin.py
    health_grade: F
  ...
  - rank: 3
    ...
    mod_count_30d: 14
    dead_symbols: 1
    target_symbol: CodeXrayMCPServer._handle_call_tool
...
uv run codexray --project-health --max-files 20 --output-format json

# 2. Dead-code candidates
uv run codexray --dead-code --output-format json

# 3. Complexity heatmap (covers the structural smell axis)
uv run codexray --overview --output-format json
...
uv run codexray <file> --table --output-format json
uv run codexray --callers <SYMBOL> --output-format json
uv run codexray <file> --refactor --output-format json
```
