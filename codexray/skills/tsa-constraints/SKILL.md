...
```bash
uv run codexray --check-constraints --output-format toon
uv run codexray --check-constraints --severity-min error
uv run codexray --check-constraints --constraint-path-filter "mcp/**"
uv run codexray --check-constraints --constraint-file path/to/alt-rules.yml
```
...
CI gating example:
```bash
uv run codexray --check-constraints || exit 1   # block PR on UNSAFE
```

## Anti-patterns

- Don't write a rule per file — use globs (`codexray/mcp/**`)
