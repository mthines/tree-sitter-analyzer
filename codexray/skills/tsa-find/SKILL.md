...
```yaml
search action=content query="TODO" roots=["codexray/"] include_globs=["*.py"]
```
...
```yaml
health action=scale file_path="codexray/ast_cache.py"
...
```bash
uv run codexray --project-root . --outline   # list project files
uv run codexray --check-tools                # verify fd/rg available
uv run codexray <file> --partial-read        # sized read with --start-line / --end-line
```
