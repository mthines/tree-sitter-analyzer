...
```bash
uv run codexray --ast-cache --ast-cache-mode index --ast-cache-force
# OR safer (preserves what works):
uv run codexray --ast-cache --ast-cache-mode index
```
...
```bash
uv run codexray --ast-cache --ast-cache-mode stats
uv run codexray --ast-cache --ast-cache-mode index --ast-cache-force
uv run codexray --ast-diff --ast-diff-file <file> --ast-diff-old-ref HEAD~5 --ast-diff-new-ref HEAD
uv run codexray --parser-readiness swift
uv run codexray --autoindex            # index action=auto
uv run codexray --full-index           # index action=full
